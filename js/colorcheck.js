...
const defaultStyle = {
  ...
  isItalic: false,
  isUnderline: false,
  isHighlighted: false, // Highlight state
};
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  inputArea.addEventListener('input', () => {
    const inputText = '&w' + inputArea.value; // Start with the default color
    outputArea.innerHTML = ''; // Clear output area
    
    const wrappedText = wrapText(inputText, 80);
    const segments = wrappedText.split(/(&\d{3}|&[rgObpcwzRGYBPWCkDIUw]|\n)/g);
    
    segments.forEach(segment => {
      if (colorMapping[segment]) {
        // Handle color codes
        currentStyle.color = colorMapping[segment].color;
        lastColorCode = segment; // Update the last color code
        ...
        currentStyle.color = currentStyle.isHighlighted && lastColorCode ? 
                             colorMapping[lastColorCode].highlight : 
                             lastColorCode ? colorMapping[lastColorCode].color : defaultStyle.color;
      } else if (segment === '&I') {
        // Handle italic
  ...
  function appendStyledText(text, style) {
    if (text === '\n') return; // Already handled line breaks
  ...
  function wrapText(text, maxLength) {
    ...
    paragraphs.forEach(paragraph => {
      let currentLine = '';
      let currentLength = 0;
      const words = paragraph.split(/(\s+)/); // Split the paragraph into words and spaces
  
      words.forEach(word => {
        const strippedWord = stripColorCodes(word);
        const wordLength = strippedWord.length;
  
        if (currentLength + wordLength > maxLength) {
          lines.push(currentLine); // Push the current line even if it's just spaces or color codes
          currentLine = word; // Start a new line with the current word
          currentLength = wordLength; // Reset the length counter
        } else {
      ...
      lines.push(currentLine); // Add the final line for this paragraph
    });
  ...
  function stripColorCodes(text) {
    // This function removes color codes to calculate word length but preserves the original input for display
    return text.replace(/&\d{3}|&[rgObpcwzRGYBPWCkDIUw]/g, '');
  }  
});